...
Save a JSON file to:
`C:\Users\Rithika\Desktop\ree-extraction-pipeline\results\bargraph\FILENAME_validation.json`
...
- `critical_failures`: list every critical failure found (missing series, wrong axis scale, missing categories) — empty list if none
