...
## STEP 8 — Save the validation report as JSON

Save a JSON file to:
`C:\Users\Rithika\Desktop\ree-extraction-pipeline\results\tables\FILENAME_validation.json`

The JSON must follow this exact structure, aligned with the TableValidationOutput schema:
...
```json
{
  ...
  "spanning_headers_correct": <true | false | null>,
  "column_accuracy": [
    {
      ...
      "mae": <number or null>,
      "rmse": <number or null>,
      "mismatched_cells": ["<row_id>", ...]
    }
  ],
  "overall_exact_match_rate": <0.0–1.0>,
...
- `structure_correct`: true only if extracted row count AND column count both match ground truth exactly
- `spanning_headers_correct`: true/false if spanning headers were present; null if none
- `column_accuracy`: one entry per column — `mae` and `rmse` for numeric columns only, null for string columns
- `overall_exact_match_rate`: proportion of ALL cells across ALL columns matching ground truth exactly
- `overall_confidence`: your overall assessment from Step 7
- `recommended_action`: accept (≥0.85, no critical failures) / review (0.60–0.84) / re-extract (<0.60 or any critical failure)
- `critical_failures`: missing rows/columns, row-column swap, landscape orientation errors — empty list if none
- `issues`: all non-critical discrepancies — empty list if none
- `summary`: one sentence summarising the overall validation outcome

---

## STEP 9 — Save the validation report as HTML
...
- The JSON file (Step 8) must be saved before the HTML report (Step 9)
